...
class VideoCallScreen extends StatefulWidget {
  ...
  VideoCallScreen({this.currentUser, this.mUser, this.channel, this.isCaller});
  ...
  _addListeners() {
    _engine!.setEventHandler(RtcEngineEventHandler(
      joinChannelSuccess: (channel, uid, elapsed) {
        log('AgoraCall joinChannelSuccess ${channel} ${uid} ${elapsed}');
      ...
      userJoined: (uid, elapsed) {
        log('AgoraCall userJoined  ${uid} ${elapsed}');
        setState(() {
          remoteUid.add(uid);
          remoteUser = true;
        });
      ...
      userOffline: (uid, reason) {
        ...
        log('AgoraCall userOffline  ${uid} ${reason}');
   ...
   checkCredits({bool? firstCheck}){
    ...
    if(widget.currentUser!.getCredits! >= Setup.coinsNeededForVideoCallPerMinute){
      ...
      widget.currentUser!.save().then((value) {
        ...
        QuickCloudCode.sendGift(authorId: widget.mUser!.objectId!, credits:  Setup.coinsNeededForVideoCallPerMinute);
