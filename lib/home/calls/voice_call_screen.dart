...
class VoiceCallScreen extends StatefulWidget {
  ...
  VoiceCallScreen({this.currentUser, this.mUser, this.channel, this.isCaller});
  ...
  _addListeners() {
    _engine!.setEventHandler(
        RtcEngineEventHandler(joinChannelSuccess: (channel, uid, elapsed) {
          log('AgoraVoiceCall joinChannelSuccess ${channel} ${uid} ${elapsed}');
          setState(() {
            isJoined = true;
          });

        }, userJoined: (uid, elapsed) {
          log('AgoraVoiceCall userJoined  ${uid} ${elapsed}');
          setState(() {
            remoteUid.add(uid);
          });
          ...
          log('AgoraVoiceCall userOffline  ${uid} ${reason}');
          _stopWatchTimer.onExecute.add(StopWatchExecute.stop);
          remoteUid.removeWhere((element) => element == uid);
        }, leaveChannel: (stats) {
          log('AgoraVoiceCall leaveChannel ${stats.toJson()}');
          setState(() {
            isJoined = false;
            remoteUid.clear();
          });
  ...
  checkCredits({bool? firstCheck}){
    ...
    if(widget.currentUser!.getCredits! >= Setup.coinsNeededForVoiceCallPerMinute){
      ...
      widget.currentUser!.save().then((value) {
        ...
        QuickCloudCode.sendGift(authorId: widget.mUser!.objectId!, credits:  Setup.coinsNeededForVoiceCallPerMinute);
            ...
            child: Row(
              children: [
                Padding(
                  ...
                  child: SvgPicture.asset(widget.isCaller! ? "assets/svg/ticket_icon.svg" : "assets/svg/dolar_diamond.svg" , height: 20, width: 20,),
                ),
                ...
                Visibility(
                  ...
                  child: Padding(
                    ...
                    child: SvgPicture.asset("assets/svg/ic_add_rounded_primary.svg", height: 24, width: 24,),
                  ),
                ),
              ],
            ),
