...
class CompleteProfileScreen extends StatefulWidget {
  ...
  static const currentStepPhotos = 8;

  UserModel? currentUser;
  int? currentStep;

  CompleteProfileScreen({Key? key, this.currentUser, this.currentStep}) : super(key: key);
  @override
...
class _CompleteProfileScreenState extends State<CompleteProfileScreen> {
  ...
  iniUserExistingData(){
    ...
    setState(() {
      ...
      if(widget.currentUser!.getBirthday != null){
        birthdayEditingController.text = QuickHelp.getBirthdayFromDate( widget.currentUser!.getBirthday!);
      ...
      if(widget.currentUser!.getShowSexualOrientation != null){
        showSexualOrientationProfile = widget.currentUser!.getShowSexualOrientation!;
      }
      ...
      if(QuickHelp.getPhotosCounter(widget.currentUser!).length >= Setup.photoNeededToRegister){
        isValidPhotos = true;
      }
  ...
  Future<void> _updateUserNow() async {
    ...
    if(fullNameEditingController.text.trim() != widget.currentUser!.getFullName){
      ...
      widget.currentUser!.username = username.toLowerCase().trim()+QuickHelp.generateShortUId().toString();
    }
    ...
    if(pictureList.isNotEmpty && pictureList.length > 0 && pictureList[0].getFile != null){
      widget.currentUser!.setAvatar = pictureList[0].getFile!;
      widget.currentUser!.setAvatar1 = pictureList[0].getFile!;
    }

    if(pictureList.isNotEmpty && pictureList.length > 1 && pictureList[1].getFile != null){
      widget.currentUser!.setAvatar2 = pictureList[1].getFile!;
    }

    if(pictureList.isNotEmpty && pictureList.length > 2 && pictureList[2].getFile != null){
      widget.currentUser!.setAvatar3 = pictureList[2].getFile!;
    }

    if(pictureList.isNotEmpty &&  pictureList.length > 3 && pictureList[3].getFile != null){
      widget.currentUser!.setAvatar4 = pictureList[3].getFile!;
    }

    if(pictureList.isNotEmpty && pictureList.length > 4 &&pictureList[4].getFile != null){
      widget.currentUser!.setAvatar5 = pictureList[4].getFile!;
    }

    if(pictureList.isNotEmpty && pictureList.length > 5 && pictureList[5].getFile != null){
    ...
    if (userResult.success) {
      ...
      QuickHelp.hideLoadingDialog(context);
      QuickHelp.goToNavigatorScreen(context, DispatchScreen(currentUser: widget.currentUser!,), route: DispatchScreen.route, back: false, finish: true);
  ...
  void doUserLogout(UserModel? userModel) async {
    ...
    if (response.success) {
      ...
      QuickHelp.goToNavigatorScreen(context, const WelcomeScreen(), finish: true, back: false, route: WelcomeScreen.route);
    } else {
      QuickHelp.hideLoadingDialog(context);
